...
with tab4:  
    ...
    #사용자가 체류 시간을 슬라이더로 선택(최소 : 5분, 최대 : 100분, 기본값 : 0)
    time_input = st.slider("체류 시간(분)", min_value = 5, max_value = 100, value = 5, step = 5)
    ...
    if st.button("온라인 전환율 예측"):
        ...
        #Plotly 시각화 적용(실제 전환율 VS 예측 전환율 비교)
        fig_ml_on = go.Figure()
        ...
        fig_ml_on.add_trace(go.Scatter(
            ...
            y = y_pred,
            mode = "markers + lines",
            marker = dict(symbol = "circle", size = 3, color = "red"),
        ...
        #Plotly 그래프 출력
        st.plotly_chart(fig_ml_on, use_container_width = True)
...
with tab5:  
    ...
    if st.button("오프라인 방문자 수 예측"):  
        ...
        fig_ml_off.update_traces(line = dict(color = "red"))    #선 색상 변경
        fig_ml_off.update_xaxes(title_text = "날짜")
        fig_ml_off.update_yaxes(title_text = "방문자 수")

        #Plotly 그래프 출력
        st.plotly_chart(fig_ml_off, use_container_width = True)                                       
